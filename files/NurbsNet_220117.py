...
class MLP_Softmax(nn.Module):
    def __init__(self, nF, nC):
        ...
        self.fc2 = nn.Linear(80, nC, bias=True) # nC = number of network classes
...
class nurbs_world(nn.Module):
    def __init__(self, cfg):
        ...
        num_net_classes = len(self.grid_sizeList)
        self.mlpSoftmax = MLP_Softmax(self.ch, num_net_classes) # numClasses = len(self.grid_sizeList)
    ...
    def forward(self, x):      
        ...
        #self.netIndex = torch.argmax(torch.sum(self.mlpSoftmax(featMax), dim = 0))
        featProbSum = torch.sum(featMaxProb, dim = 0) # featProbSum.shape = torch.Size([5])
        self.featProbSum = featProbSum
        self.netIndex = torch.argmax(featProbSum)
        self.num_grid_size = self.grid_sizeList[self.netIndex]
        self.mlpCP_W = MLP_CP_W(self.ch, self.cp_ch, self.num_grid_size**2)
        self.mlpCP_W.cuda()
        output = self.mlpCP_W(featMax)
        
        cp = torch.tanh(output[..., : (self.num_grid_size**2)*3] )
        w = torch.sigmoid(output[..., (self.num_grid_size**2)*3 : ] ) + 1e-4
        
#         print('\n', "featMaxProb : ", featMaxProb)
#         print("sum of featMaxProb : ", torch.sum(featMaxProb, dim = 0))
#         print("self.netIndex : ", self.netIndex)
        return cp, w.view(-1, self.num_grid_size, self.num_grid_size, 1)

#         elif torch.sum(mask[..., 0]) >= 18:
#             cpB = outputMasked[..., self.num_cpA*4 : self.num_cpA*4 + self.num_cpB*3 ]
#             cpB_upsampled = F.upsample(cpB.unsqueeze(0), scale_factor = 4, mode = 'nearest')
#             cp = torch.tanh(cpB_upsampled.squeeze(0))
            
#             wB = outputMasked[..., (self.num_cpA*4 + self.num_cpB * 3) : ]
#             wB_upsampled = F.upsample(wB.unsqueeze(0), scale_factor = 4, mode = 'nearest')
#             w = torch.sigmoid(wB_upsampled.squeeze(0)) + 1e-7
