...
class nurbs_world(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_cp = cfg.grid_size**2
        self.nblocks = cfg.num_block
        self.num_cp_layer = cfg.num_cp_layer
        self.cp_ch = cfg.cp_channel
        modules_cp_w = []
        ...
        modules_cp_w.append(nn.Linear(ch, self.cp_ch))
        modules_cp_w.append(nn.LeakyReLU(negative_slope=0.2))

        for i in range(self.num_cp_layer-1):
            if i != self.num_cp_layer-2:
                modules_cp_w.append(nn.Linear(self.cp_ch, self.cp_ch))
                modules_cp_w.append(nn.LeakyReLU(negative_slope=0.2))
            else:
                modules_cp_w.append(nn.Linear(self.cp_ch, self.num_cp*4))
        self.fc_cp_w = nn.Sequential(*modules_cp_w)

    def forward(self, x):
        points, xyz_and_feats = self.backbone(x)
        feats = torch.max(xyz_and_feats[0][1], dim=1)[0]
        for i in range(1, len(xyz_and_feats)):
            temp = torch.max(xyz_and_feats[i][1], dim=1)[0]
            feats = torch.cat((feats, temp), dim=1)
        feats = self.fc_cp_w(feats)
        cp = torch.tanh(feats[..., : self.num_cp * 3])
        w = torch.sigmoid(feats[..., self.num_cp * 3 : self.num_cp * 4]) + 1e-4
        return cp, w.view(-1, self.grid_size, self.grid_size, 1)
