...
class nurbs_world(nn.Module):
    def __init__(self, cfg):
        ...
        num_net_classes = len(self.grid_sizeList) # 5 -> 2로 변경 (가장 간단한 테스트를 위해)
        ...
        self.mlpSoftmax = nn.Sequential(nn.Linear(self.ch, 80, bias=True),
                                        nn.ReLU(),
                                        nn.Linear(80, num_net_classes, bias=True),
                                        nn.Softmax(dim=-1)
                                       )
    ...
    def forward(self, x):      
        ...
        self.featProbSum = torch.sum(featMaxProb, dim = 0) # featProbSum.shape = torch.Size([5])
        
        '''Get an index of selected network(netIndex)'''
        netIndex = torch.argmax(self.featProbSum)
        
        '''With netIndex, select one among multiple mlp networks(mlpCP_W) and feed forward'''
        self.mlpCP_W[netIndex].cuda()
        output = self.mlpCP_W[netIndex](featMax)
        
        self.num_grid_size = self.grid_sizeList[netIndex]
        cp = torch.tanh(output[..., : (self.num_grid_size**2)*3] )
        w = torch.sigmoid(output[..., (self.num_grid_size**2)*3 : ] ) + 1e-4
        
        return cp, w.view(-1, self.num_grid_size, self.num_grid_size, 1), netIndex
