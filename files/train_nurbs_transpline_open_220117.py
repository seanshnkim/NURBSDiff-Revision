...
from NurbsNet_220117 import nurbs_world
...
#for e in range(1):
    ...
    #for _ in range(1):
        ...
        #probability_sum_train += Transformer.featProbSum
        # probability_sum_train.detach()
        
            
        # probability_sum_train += Transformer.featProbSum
                
        optimizer_list[Transformer.netIndex].zero_grad()
        ...
        output = nurbs_list[Transformer.netIndex](output)
        ...
        optimizer_list[Transformer.netIndex].step()
        
        pbar.set_description_str(desc="[Train] Epoch {}/{}, loss: {:.7f}".format(e, config.epochs, loss.item()), refresh=True)

    with torch.no_grad():
        probability_sum_train = probability_sum_train / (config.num_train//config.batch_size)
    ## validation 과정 준비
    test_cd = []
    test_hd = []
    test_lap = []
    ...
    with torch.no_grad():
        probability_sum_val = probability_sum_val / (config.num_val//config.batch_size)
    
    writer1 = SummaryWriter(
   # "/files" +"/tensorboard/nurbs/transpline/" + terminal + "/" + model_name + "train"
    root + "tensorboard/nurbs/transpline/" + model_name)
