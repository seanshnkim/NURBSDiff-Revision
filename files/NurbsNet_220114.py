...
class MLP_CP_W(nn.Module):
    ...
    def forward(self, x):
        ...
        return res
################################################################################################################ 
def zeroUpsamplingby4(inp):
    inpUnsqzd = inp.unsqueeze(2)
    zeros = torch.zeros(inpUnsqzd.shape[0], inpUnsqzd.shape[1], inpUnsqzd.shape[2]*3).cuda()
...
class nurbs_world(nn.Module):
    ...
    def forward(self, x):      
        ...
        cpA = torch.tanh(outputMasked[..., : self.num_cpA * 3])
        cpB = torch.tanh(outputMasked[..., self.num_cpA*4 : self.num_cpA*4 + self.num_cpB*3 ] )
        cpB_upsampled = zeroUpsamplingby4(cpB)
        cp = cpA + cpB_upsampled
        
        wA = torch.sigmoid(outputMasked[..., self.num_cpA * 3 : self.num_cpA * 4]) + 1e-7
        wB = torch.sigmoid(outputMasked[..., (self.num_cpA* 4 + self.num_cpB * 3) : ]) + 1e-7
        wB_upsampled = zeroUpsamplingby4(wB)
        w = wA + wB_upsampled

        return cp, w.view(-1, self.grid_sizeA, self.grid_sizeA, 1) #, cpA, cpB_upsampled #, torch.sum(mask[...,0]).item(), torch.sum(mask[..., 1]).item()
