...
class nurbs_world(nn.Module):
    ...
    def forward(self, x):      
        ...
        self.featMaxProb = self.mlpSoftmax(featMax)
        # featMaxGS = F.gumbel_softmax(torch.log(featMaxProb, dim =1), tau = 0.1, hard = True, eps=1e-10, dim = -1)
        
        _, indices = torch.max(self.featMaxProb, dim =1)
        # featMaxList = []
        outputList = []
        cpList = []
        wList = []
        ...
        for i in range(self.num_net_classes):
            eachBatchSize.append(featMax[indices == i].shape[0] )
            
            self.mlpCP_W[i].cuda()
            num_grid_size = self.grid_sizeList[i]   
            
            out_mlpCP_W = self.mlpCP_W[i](featMax)
            
            cp = torch.tanh(out_mlpCP_W[..., : (num_grid_size**2)*3] )
            cpList.append(cp)
            
            w = torch.sigmoid(out_mlpCP_W[..., (num_grid_size**2)*3 : ] ) + 1e-4
