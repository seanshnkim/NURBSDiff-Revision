...
class nurbs_world(nn.Module):
    def __init__(self, cfg):
        ...
        self.mlpGS = MLP_gumbelSoftmax(self.ch, num_net_classes) # numClasses = len(self.grid_sizeList)
    ...
    def forward(self, x):      
        ...
        featMaxProb = self.mlpGS(featMax)
        #self.netIndex = torch.argmax(torch.sum(self.mlpSoftmax(featMax), dim = 0))
        self.netIndex = torch.argmax(torch.sum(featMaxProb, dim = 0))
